...
sqlDB.connect().then(() => console.log("DB 3 connected!")).catch(err => console.log('DB 3: ', err.message))
    ...
    run: async (client, interaction, args) => {
        ...
        sqlDB.query(query, values, async (err, res) => {
            if (err) console.log(err)
                ...
                if(res.rowCount===0) return interaction.followUp({embeds:[new Discord.MessageEmbed().setColor('RED').setDescription('**❎ | Please register first to edit your roster!**')]})
                ...
                interaction.followUp({ embeds: [embed],ephemeral:true }).then(async sent => {
                    ...
                    const filter = (reaction, user) => {
                        return (reaction.emoji.name === '0️⃣' || '1️⃣' || '2️⃣' || '3️⃣' || '4️⃣' || '5️⃣' || '6️⃣' || '7️⃣' || '⏸️' || xEmoji.name) && (user.id === interaction.user.id)
                    };
                            ...
                            if (flag === 0) {
                                ...
                                sqlDB.query(query2, values2, async (err, res) => {
                                    if (err) console.log(err)
                                        ...
                                        interaction.followUp({ embeds: [new Discord.MessageEmbed().setColor('GREEN').setDescription('✅ | Team Name Updated!')], ephemeral: true })
                                ...
                                sqlDB.query(query2, values2, async (err, res) => {
                                    if (err) console.log(err)
                                        ...
                                        interaction.followUp({ embeds: [new Discord.MessageEmbed().setColor('GREEN').setDescription('✅ | Player Details Updated!')], ephemeral: true })
