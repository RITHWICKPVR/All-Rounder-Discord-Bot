...
sqlDB.connect().then(() => console.log("DB 2 connected!")).catch(err => console.log('DB 2: ', err.message))
        ...
        if (interaction.customId === 'ticket_btn') {
            ...
            sqlDB.query(query, values, async (err, res) => {
                if (err) {
                    ...
                    if (res.rowCount === 1 && (res.rows[0].ticketstatus === 'OPEN')) {
                        ...
                        let embed = new Discord.MessageEmbed().setColor('RED').setDescription(`👮‍♂️ | **You already have a ticket open!**\n\nPlease head over to ${ticketChannel}`)
                        ...
                        sqlDB.query(query2, async (err, res1) => {
                            if (err) {
                                ...
                                await interaction.guild.channels.create(chnName, {
                                    type: "GUILD_TEXT",
                                    ...
                                    let embed = new Discord.MessageEmbed().setColor('WHITE').setDescription(`✅ | **You have successfully opened a new support ticket!**\n\nHead over to ${channel}`)
                                    ...
                                    let row = new MessageActionRow().addComponents(new MessageButton().setCustomId('delete_chnl').setStyle('DANGER').setLabel('Close'), new MessageButton().setCustomId('archive_chnl').setStyle('PRIMARY').setLabel('Archive'))


                                    const welcomeEmbed = new Discord.MessageEmbed().setColor('WHITE').setDescription(`Hello !\nOne of our moderators will be here to assist u shortly! \n\n**Please drop your query below.**`.toProperCase()).setThumbnail(interaction.member.user.avatarURL()).setImage('https://media.discordapp.net/attachments/729683661653278821/736247974832898140/ezgif-1-a2a2e7173d80-1-1_1-1-1.gif')

                                    channel.send({
                                        embeds: [welcomeEmbed], content: `${interaction.user} ||<@&922429787748851753>||`, // ||${staffRole}||
                                        components: [row]
                                    }).then(sent => {

                                        var today = new moment()

                                        let query = 'insert into tickets(authorID,authorName,messageID,ticketStatus,adminID,adminName,ticketChannel,timestamp) values($1,$2,$3,$4,$5,$6,$7,$8);'
                                        let values = [
                                            interaction.user.id,
            ...
            sqlDB.query(query2, values2, async (err, res) => {
                if (err) {
                    ...
                    let embed = new Discord.MessageEmbed().setColor('GREEN').setTitle('Ticket Resolved!').setTimestamp().setDescription(`Your ticket has been closed by ${interaction.user
                        }.`)
            ...
            let embed = new Discord.MessageEmbed().setDescription(`Thank You for using TEC Support!`).setFooter({ text: 'This channel will be deleted in 2 seconds.' }).setColor('GREEN').setTimestamp()
            ...
            let logEmbed = new Discord.MessageEmbed().setDescription(`Ticket **\`${interaction.channel.name
                }\`** deleted by ${interaction.user
                }`).setTimestamp().setColor('BLURPLE').setTimestamp()
            ...
            if (userRolesArray.some(item => allowedRoles.includes(item)) === true) {
                ...
                let row2 = new MessageActionRow().addComponents(new MessageButton().setCustomId('delete_chnl').setStyle('DANGER').setLabel('Close'))

                let archiveCateory = await client.channels.cache.find(c => c.name === 'Archived Tickets' && c.type === 'GUILD_CATEGORY')

                let embed = new Discord.MessageEmbed().setColor('GREEN').setDescription(`This channel has been archived by **\`${interaction.user.tag
                    }\`**`).setTimestamp()
                ...
                let embed = new Discord.MessageEmbed().setDescription(`👮‍♂️ | **Tickets can only be archived by <@&${config.coreBasikID}>!**`).setColor('BLURPLE')
